  ...
  Future<Map<String, dynamic>?> getCurrentUserProfile() async {
    try {
      ...
      try {
        ...
        if (response.isEmpty) {
          AppLogger.warning('Profile query returned empty for user ${user.id}');
        ...
        profile['created_at'] = profile['created_at'] ?? DateTime.now().toIso8601String();
        profile['updated_at'] = profile['updated_at'] ?? DateTime.now().toIso8601String();
        ...
        final profileData = {
          ...
          'role': 'user',
          'is_active': true,
          'is_verified': false, // Admin users should verify themselves
          'created_at': DateTime.now().toIso8601String(),
          'updated_at': DateTime.now().toIso8601String(),
        }..removeWhere((key, value) => value == null);
        ...
        // Validate the new profile has all required fields
        if (newProfile.isNotEmpty) {
          final validatedProfile = Map<String, dynamic>.from(newProfile);
          validatedProfile['id'] = validatedProfile['id'] ?? user.id;
          validatedProfile['email'] = validatedProfile['email'] ?? user.email ?? '';
          validatedProfile['full_name'] = validatedProfile['full_name'] ?? fullName;
          validatedProfile['phone_number'] = validatedProfile['phone_number'] ?? phoneNumber ?? '';
          validatedProfile['role'] = validatedProfile['role'] ?? 'user';
          validatedProfile['is_verified'] = validatedProfile['is_verified'] ?? false;
          validatedProfile['is_active'] = validatedProfile['is_active'] ?? true;
