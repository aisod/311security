...
Deno.serve(async (req) => {
    ...
    bucketConfig.allowed_mime_types = ["image/jpeg", "image/png", "image/jpg", "image/webp", "video/mp4", "video/quicktime", "application/pdf"];
    ...
    const policyQueries = [
        ...
        `CREATE POLICY "Public Access for crime-evidence" ON storage.objects FOR SELECT USING (bucket_id = 'crime-evidence');`,
        // Allow public to insert objects in this bucket
        `CREATE POLICY "Public Upload for crime-evidence" ON storage.objects FOR INSERT WITH CHECK (bucket_id = 'crime-evidence');`,
        // Allow public to update objects in this bucket
        `CREATE POLICY "Public Update for crime-evidence" ON storage.objects FOR UPDATE USING (bucket_id = 'crime-evidence');`,
        // Allow public to delete objects in this bucket
        `CREATE POLICY "Public Delete for crime-evidence" ON storage.objects FOR DELETE USING (bucket_id = 'crime-evidence');`
    ];
    ...
    return new Response(JSON.stringify({
        ...
        name: 'crime-evidence',
        public: true,
        allowed_mime_types: ["image/jpeg", "image/png", "image/jpg", "image/webp", "video/mp4", "video/quicktime", "application/pdf"],
