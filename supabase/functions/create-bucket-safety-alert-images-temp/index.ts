...
Deno.serve(async (req) => {
    ...
    const bucketConfig = {
        id: 'safety-alert-images',
        name: 'safety-alert-images',
        public: true
    };

    // Add optional configurations
    bucketConfig.allowed_mime_types = ["image/jpeg", "image/png", "image/jpg", "image/webp"];
    ...
    const policyQueries = [
        ...
        `CREATE POLICY "Public Access for safety-alert-images" ON storage.objects FOR SELECT USING (bucket_id = 'safety-alert-images');`,
        // Allow public to insert objects in this bucket
        `CREATE POLICY "Public Upload for safety-alert-images" ON storage.objects FOR INSERT WITH CHECK (bucket_id = 'safety-alert-images');`,
        // Allow public to update objects in this bucket
        `CREATE POLICY "Public Update for safety-alert-images" ON storage.objects FOR UPDATE USING (bucket_id = 'safety-alert-images');`,
        // Allow public to delete objects in this bucket
        `CREATE POLICY "Public Delete for safety-alert-images" ON storage.objects FOR DELETE USING (bucket_id = 'safety-alert-images');`
    ];
